...
export default function Assessment101() {
  ...
  function commitScore() {
     ...
     *
     * Instead of asking how many darts were used in the visit,
     * we ask how many darts were thrown at a double.
     */
    if (score === remainder) {
     ...
     * but the score remains unchanged.
     */
  ...
  function confirmCheckout(doubleDarts: number) {
    /*
     * Work out how many total darts were used in the
     * checkout visit based on:
     *
     * - the preferred checkout route
     * - the number of darts actually thrown at doubles
     */
    const checkoutDarts =
      getCheckoutDartsFromDoubleAttempts(
        remainder,
    ...
    /*
     * Every previous visit is 3 darts.
     * The final visit uses the calculated checkout darts.
     */
    const totalDarts =
  ...
  React.useEffect(() => {
    function handleKeyboard(
      ...
      /*
       * Don't allow keyboard score entry
       * while the checkout modal is open.
       */
  ...
  if (complete) {
    ...
    const totalDoubleDarts = legs.reduce(
      (sum, item) =>
